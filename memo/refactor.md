...
- shared service table の `blob_load` は実装済み。stage2 は stage3 payload の検索、header `load_addr` 適用、展開 staging の選択を `blob_load` に委譲する。互換 fallback として直接 `blob_expand` 経路は残す。
