...
struct legacy_platform_ops {
    ...
    void (*hdd_get_geometry)(struct legacy_hdd_geometry* geometry);
    int (*hdd_read_sectors)(unsigned int lba, unsigned int count,
                            unsigned int dest);
    int (*hdd_load_mbr_boot_sector)(unsigned int dest);
    int (*rtc_read_time_bcd)(unsigned char* hour_bcd, unsigned char* min_bcd,
                             unsigned char* sec_bcd);
    int (*rtc_read_date_bcd)(unsigned char* year_bcd, unsigned char* mon_bcd,
                             unsigned char* day_bcd);
    int (*rtc_set_time_bcd)(unsigned char hour_bcd, unsigned char min_bcd,
                            unsigned char sec_bcd);
