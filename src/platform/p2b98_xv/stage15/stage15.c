...
static unsigned int current_stack_top(void) {
    unsigned int esp;
...
static void install_shared_service_table(unsigned int total_bytes) {
    ...
    table->stack_top = current_stack_top();
    ...
    table->crc32 =
        service_base +
        ((unsigned int)blob_crc32_service - (unsigned int)__blob_service_start);
...
static void load_stage2_blob(unsigned int total_bytes) {
    ...
    install_shared_service_table(total_bytes);
    serial_write_string("svctab1.5 ok\r\n");
    table = shared_service_from_total(total_bytes);
    if (table != 0 && table->blob_load != 0u) {
        load = (blob_load_fn)table->blob_load;
    }
    serial_write_string("Load stage2 blob @ ");
    serial_write_hex32(STAGE2_LOAD_LINEAR);
    serial_write_string("...\r\n");
    p = (volatile unsigned int*)STAGE2_LOAD_LINEAR;
    end = (volatile unsigned int*)(STAGE2_LOAD_LINEAR + STAGE2_LOAD_CAPACITY);
    while (p < end) {
        *p++ = 0u;
    }
    if (load == 0) {
        serial_write_string("stage2 blob svc missing\r\n");
        die_with_post(0xee);
    }
    rc = load(SHARED_PAYLOAD_ID_STAGE2, (void*)STAGE2_LOAD_LINEAR,
              STAGE2_LOAD_CAPACITY, &stage2_load, &status, total_bytes);
    if (rc != 0) {
        serial_write_string("stage2 load failed rc=");
...
void stage15_main(unsigned int total_bytes) {
    serial_write_string("start stage1.5 @ ");
    serial_write_hex32(STAGE15_LOAD_LINEAR);
    serial_write_string("\r\n");
    load_stage2_blob(total_bytes);
}
